...
- **Evaluation Scripts**:
    ...
    - `make_report.py`: Generates a summary table from the evaluation results, presenting average scores and insights.
...
- To create a performance report from the evaluated feedback:
    ```bash
    python make_report.py --feedback_file_path "./feedback/evaluated.json"
    ```
